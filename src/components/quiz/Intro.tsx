    ...
    <div className="absolute inset-x-4 bottom-24 z-10 mx-auto max-w-md sm:bottom-28">
      <div className="min-h-[106px] animate-rise rounded-2xl border border-accent/20 bg-card/80 p-4 text-left shadow-[0_0_30px_hsl(var(--accent)/0.08)] backdrop-blur-xl sm:min-h-[112px]">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0"><p className="flex items-center gap-2 font-display text-xs font-bold uppercase tracking-[0.18em] text-accent"><Sparkles className="size-3" />{currentTip.title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground sm:text-sm">{currentTip.text} <span aria-hidden>{currentTip.icon}</span></p></div>
        </div>
      </div>
    </div>
    <div className="absolute inset-x-5 bottom-7 z-10 mx-auto w-full max-w-xs"><XpBar percent={progress} /><p className="mt-3 font-display text-[11px] tracking-[0.3em] text-accent">{STATUS[step]}</p></div>
  </div>;
}
