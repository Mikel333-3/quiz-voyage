...
function NotificationButton({ notifications, onToggle }: { notifications: NotificationItem[]; onToggle: () => void }) {
  return <button type="button" aria-label="Notifications" onClick={onToggle} className="tap relative grid size-10 shrink-0 place-items-center rounded-xl glass transition-transform hover:scale-105 hover:text-accent"><Bell className="size-5" />{notifications.length > 0 && <span className="absolute right-1.5 top-1.5 grid min-w-4 place-items-center rounded-full bg-destructive px-1 text-[9px] font-black text-destructive-foreground">{Math.min(9, notifications.length)}</span>}</button>;
}

function NotificationPanel({ notifications }: { notifications: NotificationItem[] }) {
  return <div className="fixed inset-x-3 top-[4.25rem] z-[70] mx-auto w-auto max-w-md overflow-hidden rounded-2xl border border-border/70 bg-card/95 p-2 shadow-2xl backdrop-blur-xl sm:inset-x-auto sm:right-6 sm:left-auto sm:top-20 sm:w-[min(22rem,calc(100vw-2rem))] lg:right-auto lg:left-[calc(16rem+1.25rem)] lg:top-6 lg:w-[min(22rem,calc(100vw-19rem))] lg:max-w-[22rem]"><div className="flex items-center justify-between px-2 py-2"><p className="font-display text-xs font-bold uppercase tracking-[0.18em] text-accent">Notifications</p><Sparkles className="size-4 shrink-0 text-accent" /></div>{notifications.length === 0 ? <p className="px-2 py-4 text-xs text-muted-foreground">Aucune nouvelle mission pour le moment.</p> : <div className="max-h-[min(60vh,28rem)] space-y-1 overflow-y-auto pr-1">{[...notifications].reverse().map((item) => <div key={item.id} className="rounded-xl bg-muted/40 p-3"><div className="flex min-w-0 gap-3"><span className="shrink-0 text-lg">{item.icon}</span><div className="min-w-0"><p className="break-words text-xs font-bold">{item.title}</p><p className="mt-1 break-words text-[11px] leading-4 text-muted-foreground">{item.text}</p></div></div></div>)}</div>}</div>;
}
  ...
  return <div className="min-h-screen">
    ...
    <div className="mx-auto flex w-full max-w-[1400px]">
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col gap-6 border-r border-border/60 px-5 py-6 lg:flex">
        <div className="flex items-center gap-3"><Link to="/" className="tap min-w-0 flex-1" onClick={() => playSound("nav")}><Logo className="text-2xl" /></Link><div className="relative shrink-0"><NotificationButton notifications={notifications} onToggle={toggleNotifications} /></div></div>
