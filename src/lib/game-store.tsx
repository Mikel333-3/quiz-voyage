...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) setPlayer({ ...DEFAULT_PLAYER, ...JSON.parse(raw) }); } catch { /* ignore */ }
    try { const rawResult = sessionStorage.getItem(LAST_RESULT_KEY); if (rawResult) setLastResult(JSON.parse(rawResult)); } catch { /* ignore */ }
  }, []);
  ...
  const finishMatch: Ctx["finishMatch"] = useCallback(({ score, correct, total, bestCombo, config: cfg }) => {
    ...
    persist(next);
    const result: MatchResult = { config: cfg, score, correct, total, bestCombo, xpGained, levelBefore, levelAfter, newBadges }; setLastResult(result); setConfig(null); try { sessionStorage.setItem(LAST_RESULT_KEY, JSON.stringify(result)); } catch { /* ignore */ } return result;
  }, [player, persist]);
