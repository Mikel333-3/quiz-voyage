...
import { ALL_EXTRA_BADGES, titleFor } from "./achievements";
...
type Ctx = { player: PlayerState; level: number; successRate: number; config: MatchConfig | null; lastResult: MatchResult | null; setConfig: (c: MatchConfig) => void; updateProfile: (patch: Pick<PlayerState, "name" | "school" | "avatar" | "bio">) => void; finishMatch: (input: { score: number; correct: number; total: number; bestCombo: number; config: MatchConfig }) => MatchResult; resetProgress: () => void };
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const finishMatch: Ctx["finishMatch"] = useCallback(({ score, correct, total, bestCombo, config: cfg }) => {
    ...
    add("first-win"); if (correct === total && total > 0) add("perfect-run"); if (bestCombo >= 5) add("streak-master"); if (bestCombo >= 10) add("combo-10"); if (cfg.mode === "chrono" && correct / Math.max(total, 1) >= 0.8) add("speedster"); if (cfg.mode === "haiti") add("haiti-explorer"); if (cfg.zoneId && !player.zonesCleared.includes(cfg.zoneId)) { /* zone progress is recorded below */ }
    ...
    persist(next);
    const result: MatchResult = { config: cfg, score, correct, total, bestCombo, xpGained, levelBefore, levelAfter, newBadges }; setLastResult(result); return result;
  }, [player, persist]);
  const value = useMemo<Ctx>(() => ({ player, level: levelFromXp(player.xp), successRate: player.totalAnswers ? Math.round((player.correctAnswers / player.totalAnswers) * 100) : 0, config, lastResult, setConfig, updateProfile, finishMatch, resetProgress: () => persist(DEFAULT_PLAYER) }), [player, config, lastResult, updateProfile, finishMatch, persist]);
