...
import { questionsForZone, HAITI_ZONE_QUESTIONS } from "@/lib/haiti-zone-questions";
...
import { getFeedbackDelay } from "@/lib/game-settings";
...
function QuizPage() {
  ...
  const [index, setIndex] = useState(0); const [selected, setSelected] = useState<number | null>(null); const [combo, setCombo] = useState(0); const [bestCombo, setBestCombo] = useState(0); const [score, setScore] = useState(0); const [seconds, setSeconds] = useState(mode.timePerQuestion ?? 0); const [answered, setAnswered] = useState(false);
  ...
  useEffect(() => { if (!config || answered || mode.timePerQuestion === null) return; if (seconds <= 0) { submit(null); return; } const id = window.setInterval(() => setSeconds((s) => { const next = Math.max(0, s - 1); if (next <= 5) playSound("tick"); return next; }), 1000); return () => window.clearInterval(id); }, [seconds, answered, config, mode.timePerQuestion]);
