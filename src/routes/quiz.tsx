...
function QuizPage() {
  ...
  useEffect(() => { timeoutHandledRef.current = false; setSeconds(questionTimer ?? 0); setSpokenAnswer(""); setListening(false); if (voiceActive) speakQuestion(questions[index]?.prompt ?? ""); return () => stopSpeaking(); }, [index, questionTimer, voiceActive, questions]);
  ...
  function toggleVoice() { const next = !voiceActive; setVoiceActive(next); setVoiceEnabled(next); if (next) speakQuestion(question.prompt); else { stopSpeaking(); recognitionRef.current?.stop(); setListening(false); } }
