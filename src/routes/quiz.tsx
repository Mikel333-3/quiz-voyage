...
import { ArrowLeft, Check, Flame, Mic, MicOff, Timer, Volume2, X, Zap } from "lucide-react";
...
import { answersMatch } from "@/lib/voice/answer-normalizer";
import { isSpeechRecognitionSupported, startRecognition } from "@/lib/voice/recognition";
import { isSpeechSynthesisSupported, isVoiceEnabled, setVoiceEnabled, speakQuestion, stopSpeaking } from "@/lib/voice/speech";
...
function QuizPage() {
  ...
  const [listening, setListening] = useState(false); const [spokenAnswer, setSpokenAnswer] = useState("");
  const scoreRef = useRef(0); const correctRef = useRef(0); const comboRef = useRef(0); const bestComboRef = useRef(0); const advancingRef = useRef(false); const timeoutHandledRef = useRef(false); const recognitionRef = useRef<ReturnType<typeof startRecognition> | null>(null);
  ...
  useEffect(() => {
    timeoutHandledRef.current = false; setSeconds(questionTimer ?? 0); setSpokenAnswer(""); setListening(false); recognitionRef.current?.stop();
    if (voiceActive && questions[index]?.prompt) {
      speakQuestion(questions[index].prompt, "fr-FR", () => {
        if (!answered && isSpeechRecognitionSupported()) listen();
      });
    }
    return () => { stopSpeaking(); recognitionRef.current?.stop(); };
  }, [index, questionTimer, voiceActive, questions]);
  ...
  function submit(answer: number | null) { if (answered) return; recognitionRef.current?.stop(); setListening(false); const correct = answer === question.correctIndex; const nextCombo = correct ? comboRef.current + 1 : 0; const gained = correct ? pointsForAnswer(nextCombo, questionTimer === null ? null : seconds, questionTimer) : 0; const nextScore = scoreRef.current + gained; const nextBest = Math.max(bestComboRef.current, nextCombo); scoreRef.current = nextScore; correctRef.current += correct ? 1 : 0; comboRef.current = nextCombo; bestComboRef.current = nextBest; setSelected(answer); setAnswered(true); setCombo(nextCombo); setBestCombo(nextBest); setScore(nextScore); playSound(correct ? "correct" : "wrong"); }
  function listen() { if (answered || listening || !isSpeechRecognitionSupported()) return; setSpokenAnswer(""); setListening(true); stopSpeaking(); recognitionRef.current = startRecognition(({ transcript }) => { setSpokenAnswer(transcript); const matchIndex = question.answers.findIndex((answer) => answersMatch(transcript, answer)); setListening(false); if (matchIndex >= 0) submit(matchIndex); else { setListening(false); playSound("wrong"); } }, () => { setListening(false); }, "fr-FR"); }
  function toggleVoice() { const next = !voiceActive; setVoiceActive(next); setVoiceEnabled(next); if (next) speakQuestion(question.prompt, "fr-FR", () => { if (!answered && isSpeechRecognitionSupported()) listen(); }); else { stopSpeaking(); recognitionRef.current?.stop(); setListening(false); } }
  function next() { if (advancingRef.current) return; advancingRef.current = true; stopSpeaking(); if (index + 1 >= questions.length) { finishMatch({ score: scoreRef.current, correct: correctRef.current, total: questions.length, bestCombo: bestComboRef.current, config }); void navigate({ to: "/resultats", replace: true }); return; } setIndex((i) => i + 1); setSelected(null); setAnswered(false); setSeconds(questionTimer ?? 0); timeoutHandledRef.current = false; advancingRef.current = false; }
  const exitMatch = () => { clearMatch(); stopSpeaking(); recognitionRef.current?.stop(); playSound("nav"); void navigate({ to: "/jouer", replace: true }); };
  const progress = ((index + (answered ? 1 : 0)) / questions.length) * 100;
  const voiceAvailable = isSpeechSynthesisSupported() || isSpeechRecognitionSupported();
  return <AppShell><div className="mx-auto max-w-3xl space-y-4"><div className="flex items-center justify-between gap-3"><button onClick={exitMatch} className="tap glass rounded-full p-2 transition-transform duration-200 hover:-translate-y-0.5 hover:scale-105" aria-label="Quitter la partie"><ArrowLeft className="size-4" /></button><div className="min-w-0 flex-1"><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-[image:var(--gradient-primary)] transition-all" style={{ width: `${progress}%` }} /></div></div><span className="font-display text-xs font-bold">{index + 1}/{questions.length}</span></div><div className="flex flex-wrap items-center justify-between gap-2 text-xs"><span className="glass flex items-center gap-1 rounded-full px-3 py-1.5"><Zap className="size-3.5 text-accent" /> {score} pts</span><span className={`glass flex items-center gap-1 rounded-full px-3 py-1.5 ${tier !== "none" ? "text-warning" : "text-muted-foreground"}`}><Flame className="size-3.5" /> x{comboMultiplier(combo)}</span>{questionTimer !== null && <span className={`glass flex items-center gap-1 rounded-full px-3 py-1.5 ${seconds <= 5 ? "text-destructive animate-glow-pulse" : "text-accent"}`}><Timer className="size-3.5" /> {seconds}s</span>}</div><Panel glow className="p-5 sm:p-7"><div className="flex items-start justify-between gap-3"><div><p className="font-display text-xs uppercase tracking-[0.25em] text-accent">{question.category ?? mode.name}</p><h1 className="mt-3 text-xl font-black leading-tight sm:text-2xl">{question.prompt}</h1></div>{voiceAvailable && <button onClick={toggleVoice} className={`tap glass rounded-full p-3 ${voiceActive ? "text-accent" : "text-muted-foreground"}`} aria-label={voiceActive ? "Désactiver le mode vocal" : "Activer le mode vocal"} title={voiceActive ? "Mode vocal activé" : "Activer le mode vocal"}>{voiceActive ? <Volume2 className="size-5" /> : <MicOff className="size-5" />}</button>}</div>{voiceActive && <div className="mt-5 rounded-2xl border border-primary/20 bg-surface-2 p-4"><div className="flex flex-col items-center gap-3 text-center"><p className="text-xs font-semibold text-muted-foreground">{listening ? "Je vous écoute…" : spokenAnswer ? `Vous avez dit : « ${spokenAnswer} »` : "La question sera lue, puis le micro démarrera automatiquement"}</p>{isSpeechRecognitionSupported() ? <button disabled={answered || listening} onClick={listen} className="tap inline-flex items-center gap-2 rounded-full bg-[image:var(--gradient-primary)] px-5 py-3 font-display text-sm font-bold shadow-lg disabled:opacity-50"><Mic className={`size-5 ${listening ? "animate-pulse" : ""}`} />{listening ? "Écoute…" : "Répondre avec le micro"}</button> : <p className="text-xs text-muted-foreground">La reconnaissance vocale n’est pas disponible sur ce navigateur. Utilisez les réponses ci-dessous.</p>}</div></div>}<div className="mt-6 grid gap-3">{question.answers.map((answer, i) => { const isCorrect = answered && i === question.correctIndex; const isWrong = answered && selected === i && !isCorrect; return <button key={`${question.id}-${i}`} disabled={answered} onClick={() => submit(i)} className={`tap rounded-2xl border p-4 text-left text-sm font-semibold transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_24px_hsl(var(--primary)/.12)] ${isCorrect ? "border-success bg-success/15" : isWrong ? "border-destructive bg-destructive/15" : "border-border bg-surface-2 hover:border-primary/60"}`}><span className="mr-3 inline-grid size-7 place-items-center rounded-lg bg-muted font-display text-xs">{String.fromCharCode(65 + i)}</span>{answer}{isCorrect && <Check className="float-right mt-1 size-5 text-success" />}{isWrong && <X className="float-right mt-1 size-5 text-destructive" />}</button>; })}</div>{answered && <div className="mt-5 animate-rise rounded-2xl bg-surface-2 p-4"><p className="font-display text-sm font-bold">{selected === question.correctIndex ? "🔥 Bonne réponse !" : "Pas cette fois."}</p><p className="mt-1 text-sm text-muted-foreground">{question.explanation}</p><p className="mt-3 text-xs text-muted-foreground">Question suivante dans {feedbackDelay / 1000} seconde{feedbackDelay / 1000 > 1 ? "s" : ""}…</p></div>}</Panel></div></AppShell>;
}
