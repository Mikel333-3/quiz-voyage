...
import { DEFAULT_FEEDBACK_DELAY, getFeedbackDelay, getVolume, isMusicEnabled, setFeedbackDelay, setMusicEnabled, setVolume } from "@/lib/game-settings";
...
function SettingsPage() {
  ...
  useEffect(() => { setSound(isSoundEnabled()); setMusic(isMusicEnabled()); setVolumeState(getVolume()); setDelay(getFeedbackDelay()); }, []);
  ...
  return <AppShell><div className="mx-auto max-w-2xl space-y-5">
    ...
    <Panel className="p-5"><SectionTitle title="Déroulement" /><div className="rounded-2xl bg-surface-2 p-4"><div className="flex items-center justify-between gap-3"><span className="flex items-center gap-3"><Timer className="size-5 text-accent" /><span><b className="block text-sm">Temps après une réponse</b><small className="text-muted-foreground">Avant de passer automatiquement à la question suivante</small></span></span><span className="font-display text-sm font-bold text-accent">{delay}s</span></div><input aria-label="Temps après une réponse" type="range" min="1" max="10" step="1" value={delay} onChange={(e) => changeDelay(Number(e.target.value))} className="mt-4 w-full accent-[hsl(var(--primary))]" /><div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>1 s</span><span>Par défaut : 3 s</span><span>10 s</span></div></div></Panel>
