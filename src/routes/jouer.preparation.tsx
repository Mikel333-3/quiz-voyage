...
export const Route = createFileRoute("/jouer/preparation")({
  ...
  head: () => ({ meta: [{ title: "Prépare ta partie — QuizTime Go" }, { name: "description", content: "Configure ta mission QuizTime Go." }] }),
  component: Preparation,
});
function Chip({ active, children, onClick }: { active: boolean; children: ReactNode; onClick: () => void }) { return <button onClick={onClick} className={cn("tap rounded-xl border px-4 py-2.5 text-sm font-semibold transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.02]", active ? "border-transparent bg-[image:var(--gradient-primary)] text-primary-foreground" : "border-border bg-surface-2 text-muted-foreground hover:border-primary/50")}>{children}</button>; }
function Preparation() {
  ...
  const launch = () => { const finalSubject: SubjectId | "haiti" = isHaiti ? "haiti" : random ? (shuffle(SUBJECTS)[0]!.id as SubjectId) : subject; setConfig({ mode: mode.id, subject: finalSubject, level: random ? shuffle([...LEVELS])[0]! : level, difficulty: random ? shuffle([...DIFFICULTIES])[0]! : difficulty, questionCount: count }); navigate({ to: "/quiz", replace: true }); };
  return <AppShell><div className="mx-auto max-w-3xl space-y-5 pb-28"><button onClick={() => navigate({ to: "/jouer", replace: true })} className="tap glass inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold transition-transform duration-200 hover:-translate-y-0.5"><ArrowLeft className="size-4" /> Modes</button><Panel glow className="p-5"><p className="font-display text-xs uppercase tracking-[0.35em] text-accent">Ready ?</p><h1 className="mt-2 text-2xl font-black">{mode.name}</h1><p className="mt-1 text-sm text-muted-foreground">{mode.description}</p><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="glass rounded-full px-3 py-1.5">{mode.timePerQuestion === null ? "Sans chrono" : `${configuredTimer}s / question`}</span><span className="glass rounded-full px-3 py-1.5">XP x{mode.xpMultiplier}</span><span className="glass flex items-center gap-1 rounded-full px-3 py-1.5 text-accent"><Zap className="size-3.5" /> ~{Math.round(count * 130 * mode.xpMultiplier)} XP</span></div></Panel>{random ? <Panel className="p-5 text-sm text-muted-foreground">🎲 Le système choisira automatiquement la matière, le niveau et la difficulté.</Panel> : !isHaiti && <section><SectionTitle title="Matière" /><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{SUBJECTS.map((s) => <Chip key={s.id} active={subject === s.id} onClick={() => setSubject(s.id)}>{s.name}</Chip>)}</div></section>}{!random && <><section><SectionTitle title="Niveau" /><div className="flex flex-wrap gap-2">{LEVELS.map((l) => <Chip key={l} active={level === l} onClick={() => setLevel(l)}>{l}</Chip>)}</div></section><section><SectionTitle title="Difficulté" /><div className="flex flex-wrap gap-2">{DIFFICULTIES.map((d) => <Chip key={d} active={difficulty === d} onClick={() => setDifficulty(d)}>{d}</Chip>)}</div></section></>}<section><SectionTitle title="Nombre de questions" /><div className="flex flex-wrap gap-2">{QUESTION_COUNTS.map((c) => <Chip key={c} active={count === c} onClick={() => setCount(c)}>{c} questions</Chip>)}</div></section><div className="sticky bottom-4 z-20 -mx-2 rounded-2xl border border-border/70 bg-background/90 p-3 shadow-2xl backdrop-blur-xl"><div className="grid gap-2 sm:grid-cols-2"><GhostButton onClick={() => navigate({ to: "/", replace: true })}>Retour au lobby</GhostButton><PrimaryButton onClick={launch}>Lancer la partie</PrimaryButton></div></div></div></AppShell>;
}
